...
if os.path.exists('calibration_data.txt'):
    response = input("Calibration data found. Recalibrate? (y/n): ")
    if response.lower() == 'y':
        recalibrate = True
    else:
        recalibrate = False
        ...
        with open('calibration_data.txt', 'r') as f:
            lines = f.readlines()
            min_ratio_x = float(lines[0].split(':')[1].strip())
            max_ratio_x = float(lines[1].split(':')[1].strip())
            min_ratio_y = float(lines[2].split(':')[1].strip())
            max_ratio_y = float(lines[3].split(':')[1].strip())
...
if recalibrate:
    ...
    with open('calibration_data.txt', 'w') as f:
        f.write(f"min_ratio_x: {min_ratio_x}\n")
        f.write(f"max_ratio_x: {max_ratio_x}\n")
        f.write(f"min_ratio_y: {min_ratio_y}\n")
        f.write(f"max_ratio_y: {max_ratio_y}\n")
